...
class AdminFacilities extends CI_Model {
    ...
    public function deletefacilitiesValue($indutryID) {
        $this->db->where('id', $indutryID);
        $this->db->delete($this->table);
        return $this->db->affected_rows();
    }

    public function editfacilities($data) {
    ...
    public function viewfacilities($id) {
        $this->db->select('id,name,description,createdDate,updatedDate,status');
        $this->db->from($this->table);
        $this->db->where('id', $id);
        $query = $this->db->get();
        return $query->row();
    ...
    public function updateStatus($id = array(), $status) {
        ...
        if ($wrong == true) {
            return array("status" => "NOT OK", "message" => "Selected Facilities Status Not Updated Sucessfully");
        } else {
            return array("status" => "OK", "message" => "Selected Facilities Status Updated Successfully");
        }
    }
