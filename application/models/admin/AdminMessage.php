...
class AdminMessage extends CI_Model {
	
	var $table = 'conversation';
  	var $select_fields = 'id,sender,receiver,subject,createdDate,status,message	';
   	var $where_condition = "id!='0'";
    var $column_order = array('id','id','sender','receiver','subject','createdDate','status','message	',null); //set column field database for datatable orderable
    var $column_search = array('id','sender','receiver','subject','createdDate','status','message	'); //set column field database for datatable searchable just firstname , lastname , address are searchable
	 ...
	 private function get_datatables_query()
		{
			...
			if(isset($_POST['name']) and $_POST['name']!='')
			{
				...
				$this->db->like('subject',$_POST['name']);
			}
	...
	public function deleteMessageValue($messageID)
	{
	...
	{
		$this->db->select('id,sender,receiver,subject,createdDate,status,message');
		$this->db->from($this->table);
		$this->db->where('id',$id);
		$query = $this->db->get();		
		return $query->row();
