<?php
class AdminBanner extends CI_Model {
	...
	var $table = 'home_banner';
  	var $select_fields = 'id,caption,url,image,createdDate,updatedDate,status';
   	var $where_condition = "status!='Delete'";
    var $column_order = array('id','id','image','caption','url','createdDate','updatedDate','status',null); //set column field database for datatable orderable
    var $column_search = array('id','caption','url','createdDate','updatedDate','status'); //set column field database for datatable searchable just firstname , lastname , address are searchable
	 ...
	 private function get_datatables_query()
		{
			...
			if(isset($_POST['caption']) and $_POST['caption']!='')
			{
				...
				$this->db->like('caption',$_POST['caption']);
			}
			if(isset($_POST['url']) and $_POST['url']!='')
			{
				//$this->db->where('fname="'.($_POST['fname']).'"');
				$this->db->like('url',$_POST['url']);
			}
	...
	public function deleteBanner($bannerId)
	{
		...
		@unlink("./uploads/banner/big/".$image->image);
		@unlink("./uploads/banner/thumb/".$image->image);
	...
	public function updateStatus($id = array(), $status)
	{
		...
		if($wrong == true)
		{
			return array("status"=>"NOT OK","message"=>"Selected Home Banner Status Not Updated Sucessfully");
		}
		else
		{
			return array("status"=>"OK","message"=>"Selected Home Banner Status Updated Successfully");
		}
	}	
}
?>